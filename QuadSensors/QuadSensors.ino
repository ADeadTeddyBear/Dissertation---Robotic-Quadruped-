...
#define FIRMWARE_BUILD "QuadSensors build 2026-07-25-e (VL53L0X)"
...
// computeRearJointsForHeight() can still reach for all four legs --
// the exact same per-leg HIP_MIN/MAX and KNEE_MIN/MAX validation
// setBodyHeight() itself uses, just binary-searched here to find the
// floor instead of testing one height. Computed at boot rather than
// hardcoded, so it stays correct if HIP_TRIM/KNEE_START are ever
// recalibrated -- no magic number to remember to update.
//
// At today's calibration this floor is governed entirely by the rear
// hip approaching HIP_MIN (0) -- the rear knee and both front joints
// stay comfortably mid-range at that point, so this isn't the "both
// hip and knee bent to their limits at once" case the SAFETY note
// above solveLegIK() flags as unconfirmed; only one joint per leg
// nears a limit, and each of those limits was individually confirmed
// safe by testing (see the comments on HIP_MIN/HIP_MAX). Still,
...
bool allLegsReachHeight(float heightMM) {
  ...
  return computeFrontJointsForHeight(FL, heightMM, hip, knee) &&
         ...
         computeRearJointsForHeight(RL, heightMM, hip, knee) &&
         computeRearJointsForHeight(RR, heightMM, hip, knee);
}
